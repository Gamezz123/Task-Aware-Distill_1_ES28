...
def main():
    ...
    # in the environment
    accelerator = (
        Accelerator(log_with=args.report_to, mixed_precision=args.mixed_precision, project_dir=args.output_dir) if args.with_tracking else Accelerator(mixed_precision=args.mixed_precision)
    )
    # print(accelerator)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        try:
            ...
            logger.error(f"Error during pushing to hub: {e}")
            traceback.print_exc()
        ...
        try:
            ...
            logger.error(f"Error during saving best model: {e}")
            traceback.print_exc()
    try:
        if args.with_tracking:
            accelerator.end_training()
    except Exception as e:
        logger.error(f"Error during ending tracking: {e}")
        traceback.print_exc()
    ...
    try:
        ...
        logger.error(f"Error during final evaluation on mismatched validation set: {e}")
        traceback.print_exc()
